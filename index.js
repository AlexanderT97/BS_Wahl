...
const Code = mongoose.model('Code', codeSchema, 'codes'); // Verwendung der Kollektion "codes"

const voteSchema = new mongoose.Schema({ code: String, voteOption: String });
const Vote = mongoose.model('Vote', voteSchema, 'votes'); // Verwendung der Kollektion "votes"

const resultSchema = new mongoose.Schema({ option: String, count: { type: Number, default: 0 } });
const Result = mongoose.model('Result', resultSchema, 'results'); // Verwendung der Kollektion "results"
